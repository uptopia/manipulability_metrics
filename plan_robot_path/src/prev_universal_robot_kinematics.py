...
if __name__ == '__main__':
  ...
  target_frame = PyKDL.Frame(PyKDL.Rotation.Quaternion(0.7071, -0.7071, 0.0042,0.0042),\
                            PyKDL.Vector(0.1333, 0.4914, 0.4831))
  print(target_frame, type(target_frame))
  print("\033[33mRotation Matrix from quat: \n", PyKDL.Rotation.Quaternion(0.7071, -0.7071, 0.0042,0.0042))
  rot = PyKDL.Rotation.Quaternion(0.7071, -0.7071, 0.0042,0.0042)
  print(rot[0,0])
  desired_pos = mat(np.identity(4))
  # print(desired_pos[0:3,0:3])
  # desired_pos[0:3,0:3]=PyKDL.Rotation.Quaternion(0.7071, -0.7071, 0.0042,0.0042)
  for i in range(3):
      for j in range(3):
            desired_pos[i,j]=rot[i,j]
  desired_pos[0,3] =0.1333
  desired_pos[1,3] =0.4914
  desired_pos[2,3] =0.4831
  print("desired_pose:\n", desired_pos)

  tt = invKine(desired_pos)
  # print(tt)
  # print(HTrans(tt,))

  solution_index = 0  # Index of the solution (0 to 7)
  theta1 = tt[0, solution_index]*180.0/pi
  theta2 = tt[1, solution_index]*180.0/pi
  theta3 = tt[2, solution_index]*180.0/pi
  theta4 = tt[3, solution_index]*180.0/pi
  theta5 = tt[4, solution_index]*180.0/pi
  theta6 = tt[5, solution_index]*180.0/pi
  print(theta1, theta2, theta3, theta4, theta5, theta6)

# oveit      trans: 0.1333, 0.4914, 0.4831;	rot: 0.7071, -0.7071, 0.0042,0.0042
# tf_listener trans: 0.1333, 0.4914, 0.4831;	rot: 0.7071, -0.7071, 0.0042,0.0042
# Inverse Kinematics SUCCEEDED 0
# joint_angles1: 59.63, -4.06, -90.56, 5.20, -90.34, 59.63
# Inverse Kinematics SUCCEEDED 0
# joint_angles2: 59.63, -4.06, -90.56, 5.20, -90.34, 59.63
# joint_angles3: -89.91, -90.05, -90.50, -90.13, 90.00, 90.08
# actual_joints: -90.00, -90.09, -90.45, -90.15, 89.99, 90.00